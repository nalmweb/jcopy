	...
	if ($params ['friends_only'] == 'true') {
		...
		foreach ( $aFriendsList as $item ) {
			if (null !== $item->getFriend ()->getLatitude () && null !== $item->getFriend ()->getLongitude ()) {
				...
				$out .=  " 
				       	{\"name\": \"". addslashes( $item->getFriend()->getNikname() ). "\",\n
		...
		foreach ( $aUsersList as $item ) {
			if (null !== $item->getLatitude () && null !== $item->getLongitude ()) {
				...
				{				
				  $out .= " 
				       	{\"name\": \"  ". addslashes( $item->getNikname() ). " \",\n
						 \"icon\": [\"$helmetImg\", \"$helmetImgShadow\"],\n
...
//$outputLayer0 .
 ...
 // $objResponse->addAssign('outputLayer','value',$outputLayer);
 // strpos
 // fdump( $outputLayer );
 // $objResponse->addAlert($outputLayer);
 // print $outputLayer; /* debugger;*/
 $out = 'outputLayer = ' . $output . '; reloadMarkers() ' ;
 $objResponse->addScript ( $out ); 
 fdump($out);
// $objResponse->addScriptCall ( 'reloadMarkers();' );
?>