...
EditInPlace.defaults = {
	...
	empty_text:				'Нажмите, чтобы редактировать',
	empty_class:			'Нажмите, чтобы редактировать',
	edit_title:				'Click To Edit',
	click:					'click',  // or dblclick 
	savefailed_text:		'Сохранение невозможно.',
	ajax_data:				false,
...
EditInPlace._saveClick = function(event, id)
{
	...
	// и все из=-за этой функции!!!!!!!
	var new_text =document.getElementById(id+'_edit').value; //id_opt['escape_function']($F(id + '_edit'));
 	//	alert(new_text);

	//new_text = id_opt['escape_function']($F(id + '_edit'));
	...
	var db_id = id_opt['ajax_data']['db_id'];
	...
	// xajax_editCategory();
	// Here Use ajax Instead
	// alert('saving');
	// xajax_editCategory(new_text,db_id);
