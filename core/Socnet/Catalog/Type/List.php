<?php
class Socnet_Catalog_Type_List extends Socnet_Abstract_List 
{
    public function getList()
    {
        ...
        if ( $this->isAsAssoc() ) {
            ...
            $query->from('catalog__type_auto', $fields);  
        } else {
            $query->from('catalog__type_auto', 'id');
        }
    ...
    {
        $query = $this->_db->select();
        $query->from('catalog__type_auto', 'COUNT(*)');
        if ( $this->getWhere() ) $query->where($this->getWhere());
