<?php
class Socnet_Catalog_TypeProperty_List extends Socnet_Abstract_List 
{
    public function getList()
    {
        ...
        if ( $this->isAsAssoc() ) {
            ...
            $query->from('catalog__type_property', $fields);  
        } else {
            $query->from('catalog__type_property', 'id');
        }
    ...
    {
        $query = $this->_db->select();
        $query->from('catalog__type_property', 'COUNT(*)');
        if ( $this->getWhere() ) $query->where($this->getWhere());
