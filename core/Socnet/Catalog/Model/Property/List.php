<?php
class Socnet_Catalog_Model_Property_List extends Socnet_Abstract_List 
{
    private $idModel;
    private $idProperty;
   ...
   /**
    ...
    public function getList()
    {
        ...
        if ( null !== $this->getIdModel()) {
            $query->where('cmp.id_model = ?', $this->getIdModel() );
        }
    ...
    {
        $query = $this->_db->select();
        $query->from(array('cmp'=>'catalog__model_property'), 'COUNT(*)');
        $query->join(array('cp'=>'catalog__property'), 'cmp.id_property = cp.id', array() );
        
        if ( null !== $this->getIdModel()) $query->where('cmp.id_model = ?', $this->getIdModel() );
        if ( null !== $this->getIdProperty()) {
            $query->where('cmp.id_property = ?', $this->getIdProperty() );
        }        
        if ( $this->getBboardView() ) {
