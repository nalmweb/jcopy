<?
// TODO: each class must specify table name!!!
//****************************************************************************
// phpDBTree 1.4
//****************************************************************************
...
//****************************************************************************
class Socnet_DBTree extends Socnet_Data_Entity
{
	// var $db;	// CDatabase class to plug to
	private $table;	// Table with Nested Sets implemented
	private $id = 'cid';	// Name of the ID-auto_increment-field in the table.
...
//					'level' => 'cat_level'
//				 )
    public function __construct($tableName, $itemId, $fieldNames=array())
	{
		$this->_db = Zend_Registry::get("DB");
		
		if(empty($tableName))
		{
		  $tableName = "comment__news";			
		}
		// parent::__construct($tableName);
	    // if(empty($tableName))
		//		 trigger_error("phpDbTree: Unknown table", E_USER_ERROR);
		// if(empty($itemId))
		//		 trigger_error("phpDbTree: Unknown ID column", E_USER_ERROR);
	   // $this->_db = Zend_Registry::get("DB");
		$this->table = $tableName;
		
		if(empty($itemId)) $itemId =$this->id;
		else 			   $this->id = $itemId;
		
		// устанавливаем переменные $left, $right и т.п.
...
// $ID : an ID of the element
	...
	{
		$sql = $this->_db->select()
							->from(array('t' => $this->table),array("$this->left","$this->right","$this->level"))
							->where("{$this->id} = ? ", $ID);
	  	...
	  	if(true
		//	&& ($this->_db->num_rows($query) == 1)
		//	   ($this->num_rows($query) == 1 )
			&& (!empty($Data)))
			{
			  $left = (int)$Data[$this->left];
			  // echo "[left= $left]";
			  return array($left,
						   (int)$Data[$this->right],
						   (int)$Data[$this->level]);
	...
	function my_getNodeInfo($ID)
	{
		// echo "safsdfsd f <br> ";
	    $this->sql = 'SELECT '.$this->left.','
	  						  .$this->right.','
	  						  .$this->level
	  						  .' FROM '
	  						  .$this->table
	  	...
	  	//   " sql = [$sql] <br> "	;
	  	$res =  $query=$this->_db->query($this->sql);
	  	// echo "' res = [$res] <br> ";
		if(($res)
			&& ($this->_db->num_rows($res) == 1)
			&& ($Data = $this->_db->fetch_array($res)))
			{
			   return array((int)$Data	[$this->left],
				 		    (int)$Data[$this->right],
						    (int)$Data[$this->level]);
			}
...
// $data : optional argument with data for the root node
	function clear($data=array()) {
		// clearing table
		if((!$this->_db->query('TRUNCATE '.$this->table)) &&
		   (!$this->_db->query('DELETE FROM '.$this->table)))
	...
	function update($ID, $data) {
		...
		return $this->_db->query('UPDATE '.$this->table.' SET '.substr($sql_set,1).' WHERE '.$this->id.'=\''.$ID.'\'');
	}
	//************************************************************************
	...
	function insert($ID, $data)
	{
        ...
        if ( null == $rightId ) $rightId = 0;
        if ( null == $leftId )  $leftId = 0;
		 //  echo "!";
		// preparing data to be inserted
		if(sizeof($data)) {
			$fld_names = implode(',', array_keys($data)).',';
		...
		if($ID)
		{
			$this->sql = 'UPDATE '.$this->table.' SET '
		     ...
		     // echo " sql = ". $this->sql." <br> exit;";
			if(!($this->_db->query($this->sql)))
					trigger_error("phpDbTree error: ".$this->_db->error(), E_USER_ERROR);
		}
   ...
   function moveAll($ID, $newParentId)
   {
      ...
      if(!(list($leftIdP, $rightIdP, $levelP) = $this->getNodeInfo($newParentId)))
      			 trigger_error("phpDbTree error: ".$this->_db->error(), E_USER_ERROR);

      if($ID == $newParentId || $leftId == $leftIdP ||
      ...
      {
         $this->sql = 'UPDATE '.$this->table.' SET '
            .$this->level.'=IF('.$this->left.' BETWEEN '.$leftId.' AND '.$rightId.', '
            .$this->level.sprintf('%+d', -($level-1)+$levelP).', '.$this->level.'), '
            .$this->right.'=IF('.$this->right.' BETWEEN '.($rightId+1).' AND '.($rightIdP-1).', '
            .$this->right.'-'.($rightId-$leftId+1).', '
            .'IF('.$this->left.' BETWEEN '.($leftId).' AND '.($rightId).', '.$this->right.'+'
            ...
            .'IF('.$this->left.' BETWEEN '.$leftId.' AND '.($rightId).', '
            .$this->left.'+'.((($rightIdP-$rightId-$level+$levelP)/2)*2 + $level - $levelP - 1)
            .', '.$this->left. ')) '
      ...
      elseif($leftIdP < $leftId)
      {
         ...
         .$this->level.'=IF('.$this->left.' BETWEEN '.$leftId.' AND '.$rightId.', '
         .$this->level.sprintf('%+d', -($level-1)+$levelP).', '.$this->level.'), '
         .$this->left.'=IF('.$this->left.' BETWEEN '.$rightIdP.' AND '.($leftId-1).', '
         .$this->left.'+'.($rightId-$leftId+1).', '
         . 'IF('.$this->left.' BETWEEN '.$leftId.' AND '.$rightId.', '.$this->left.'-'
         .($leftId-$rightIdP).', '.$this->left.') '. '), '
         .$this->right.'=IF('.$this->right.' BETWEEN '.$rightIdP.' AND '.$leftId.', '
         .$this->right.'+'.($rightId-$leftId+1).', '
         . 'IF('.$this->right.' BETWEEN '.$leftId.' AND '.$rightId.', '.$this->right.'-'.
         ($leftId-$rightIdP).', '.$this->right.') '
         . ') '
      ...
      else
      {
         ...
         .$this->level.'=IF('.$this->left.' BETWEEN '.$leftId.' AND '.$rightId.', '
         .$this->level.sprintf('%+d', -($level-1)+$levelP).', '.$this->level.'), '
         .$this->left.'=IF('.$this->left.' BETWEEN '.$rightId.' AND '.$rightIdP.', '
         .$this->left.'-'.($rightId-$leftId+1).', '
         . 'IF('.$this->left.' BETWEEN '.$leftId.' AND '.$rightId.', '.$this->left.'+'.
         ($rightIdP-1-$rightId).', '.$this->left.')'  . '), '
         . $this->right.'=IF('.$this->right.' BETWEEN '.($rightId+1).' AND '.($rightIdP-1).', '
         .$this->right.'-'.($rightId-$leftId+1).', '
         . 'IF('.$this->right.' BETWEEN '.$leftId.' AND '.$rightId.', '
         .$this->right.'+'.($rightIdP-1-$rightId).', '.$this->right.') '
            . ') '
      ...
      return $this->_db->query($this->sql)
      				    or trigger_error("phpDbTree error: ".$this->_db->error(), E_USER_ERROR);
   }
...
// Returns : a result id for using with other DB functions
	...
	function justGetChildren($ID)
	{		
	   // 0 - left, 2 - right
	   $data = $this->getNodeInfo($ID);
	   $sql = "SELECT * FROM ".$this->table." WHERE cleft > {$data[0]} AND  cright < {$data[1]} ORDER BY  cleft";	
	...
	function getComments($ID){
	   ...
	   foreach ($comments as $value)
  	   	 			$comments[$i++]['clevel'] *=10;
	   return $comments;
	}

	 // must set end level!! 
	 function my_enumChildren($ID, $start_level=1, $end_level=50)
	 {
	 	if($start_level < 0)
					trigger_error("phpDbTree error: ".$this->_db->error(), E_USER_ERROR);
		...
		$this->sql = $this->sqlComposeSelect(array(
			"{$this->table}.title, {$this->table}.comment,{$this->table}.user_id,{$this->table}.user_nickname,{$this->table}.registration_date,
 			 {$this->table}.cleft, {$this->table}.cright, {$this->table}.clevel, ", // Params
			'', // Fields
			$this->table.' _'.$this->table.', '.$this->table, // Tables
			'_'.$this->table.'.'.$this->id.'=\''.$ID.'\''
				.' AND '.$this->table.'.'.$this->left.' BETWEEN _'.$this->table.'.'.$this->left.' AND _'.$this->table.'.'.$this->right
				...
				echo "ASDf".$this->sql;
		return $this->_db->fetchAll($this->sql);
	}

	 // returns children that are not in list....
	 /**
	  * TODO: implement db_error method.
	  *
	  * @param unknown_type $ID
	  * @param unknown_type $aChildList
	  * @param unknown_type $WAY   - set NOT IN or just IN list.
	  * @param unknown_type $start_level
	  * @param unknown_type $end_level
	  * @return unknown
	  */
	 function enumChildrenDiffer($ID,$aChildList, $WAY, $start_level=1, $end_level=1)
	 {
	 	if($start_level < 0)
					trigger_error("phpDbTree error: ".$this->_db->error(), E_USER_ERROR);
		...
		if(!$end_level) $whereSql = $whereSql1.'>='.$whereSql2.(int)$start_level;
			...
			$whereSql = ($end_level <= $start_level)
				...
				: ' AND '.$this->table.'.'.$this->level.' BETWEEN _'.$this->table.'.'.$this->level.'+'.(int)$start_level
					.' AND _'.$this->table.'.'.$this->level.'+'.(int)$end_level
					.' AND '.$WAY.' IN ('.$aChildList.')';
		}

		$this->sql = $this->sqlComposeSelect(array(
			"{$this->table}.title, {$this->table}.value,
 			 {$this->table}.cleft, {$this->table}.cright, {$this->table}.clevel, ", // Params
			'', // Fields
			$this->table.' _'.$this->table.', '.$this->table, // Tables
			'_'.$this->table.'.'.$this->id.'=\''.$ID.'\''
	...
	/*
		@param: $node - a row of data.
		@returns:
			TRUE: - the node ($NODE_ID) has children
		   FALSE: - no children
        @note: the node has no children if #cright - #cleft=1
	*/
	function hasChildren ($node)
	{
	   if(isset($node['cright']) && isset ($node['cleft']))
		  if( $node['cright'] - $node['cleft'] > 1 )
			 return true;
		return false;
	}
	
	function hasChildrenById($ID)
	{
		if(intval($ID)>0)
		{
	...
	// $showRoot : whether to show root node in a path
	// Returns : a result id for using with other DB functions
	function myenumPath($ID, $showRoot=false) {
		//$this->qryParams = 'title';
		//$this->qryFields = 'title';
		//$this->table

		$this->sql = $this->sqlComposeSelect(array(
			$this->table.'.title,', // Params
			'', // Fields
			$this->table.' _'.$this->table.', '.$this->table, // Tables
			'_'.$this->table.'.'.$this->id.'=\''.$ID.'\''
				.' AND _'.$this->table.'.'.$this->left.' BETWEEN '.$this->table.'.'.$this->left.' AND '.$this->table.'.'.$this->right
...
// $level : Relative level of parent
// Returns : a result id for using with other DB functions
	function mygetParent($ID, $level=1)
	{
		if($level < 1) trigger_error("phpDbTree error: ".$this->_db->error(), E_USER_ERROR);
	...
	// get all info about parent
	function mygetParentALL($ID, $level=1)
	{
		if($level < 1) trigger_error("phpDbTree error: ".$this->_db->error(), E_USER_ERROR);
...
//************************************************************************
	function sqlParams($param='') { return (empty($param)) ? $this->qryParams :
															 $this->qryParams = $param; }
	function sqlFields($param='') { return (empty($param)) ? $this->qryFields :
															 $this->qryFields = $param; }
	function sqlSelect($param='') { return $this->sqlFields($param); }
	function sqlTables($param='') { return (empty($param)) ? $this->qryTables :
														     $this->qryTables = $param; }
...
//************************************************************************
	...
	{
		$joinTypes = array( 'join'=>1, 'cross'=>1, 'inner'=>1,
							'straight'=>1, 'left'=>1, 'natural'=>1, 'right'=>1);
		...
		if((!empty($arSql[3])) || (!empty($this->qryWhere))) {
			...
			if(!empty($this->qryWhere)) $this->sql .= (empty($arSql[3]))
							? $this->qryWhere
							: 'AND('.$this->qryWhere.')';
		}
		...
		if((!empty($arSql[4])) || (!empty($this->qryGroupBy))) {
			...
			if(!empty($this->qryGroupBy)) $this->sql .= (empty($arSql[4]))
							? $this->qryGroupBy
							: ','.$this->qryGroupBy;
		}
		if((!empty($arSql[5])) || (!empty($this->qryHaving))) {
			$this->sql .= ' HAVING ' . $arSql[5] . ' ';
			if(!empty($this->qryHaving)) $this->sql .= (empty($arSql[5]))
							? $this->qryHaving
							: 'AND('.$this->qryHaving.')';
		}
		...
		if((!empty($arSql[6])) || (!empty($this->qryOrderBy))) {
			...
			if(!empty($this->qryOrderBy)) $this->sql .= (empty($arSql[6]))
							? $this->qryOrderBy
							: ','.$this->qryOrderBy;
		}
