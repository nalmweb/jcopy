...
class xajax
{
	 ...
	 */
	/**
	 * @public array Array of PHP functions that will be callable through javascript wrappers
	 */
	public $aFunctions;
	/**
	 * @public array Array of object callbacks that will allow Javascript to call PHP methods (key=function name)
	 */
	public $aObjects;
	/**
	 * @public array Array of RequestTypes to be used with each function (key=function name)
	 */
	public $aFunctionRequestTypes;
	/**
	 * @public array Array of Include Files for any external functions (key=function name)
	 */
	public $aFunctionIncludeFiles;
	/**
	 * @public array Array of RequestURI to be used with each function (key=function name)
	 */
	public $aFunctionRequestURIs;
	/**
	 * @public string Name of the PHP function to call if no callable function was found
	 */
	public $sCatchAllFunction;
	/**
	 * @public string Name of the PHP function to call before any other function
	 */
	public $sPreFunction;
	/**
	 * @public string The URI for making requests to the xajax object
	 */
	public $sRequestURI;
	/**
	 * @public string The prefix to prepend to the javascript wraper function name
	 */
	public $sWrapperPrefix;
	/**
	 * @public boolean Show debug messages (default false)
	 */
	public $bDebug;
	/**
	 * @public boolean Show messages in the client browser's status bar (default false)
	 */
	public $bStatusMessages;
	/**
	 * @public boolean Allow xajax to exit after processing a request (default true)
	 */
	public $bExitAllowed;
	/**
	 * @public boolean Use wait cursor in browser (default true)
	 */
	public $bWaitCursor;
	/**
	 * @public boolean Use an special xajax error handler so the errors are sent to the browser properly (default false)
	 */
	public $bErrorHandler;
	/**
	 * @public string Specify what, if any, file xajax should log errors to (and more information in a future release)
	 */
	public $sLogFile;
	/**
	 * @public boolean Clean all output buffers before outputting response (default false)
	 */
	public $bCleanBuffer;
	/**
	 * @public string String containing the character encoding used
	 */
	public $sEncoding;
	/**
	 * @public boolean Decode input request args from UTF-8 (default false)
	 */
	public $bDecodeUTF8Input;
	/**
	 * @public boolean Convert special characters to HTML entities (default false)
	 */
	public $bOutputEntities;
	/**
	 * @public array Array for parsing complex objects
	 */
	public $aObjArray;
	/**
	 * @public integer Position in $aObjArray
	 */
	public $iPos;
	 ...
	 * For an object instance method, use an object publiciable for the second
	 * array element (and in PHP 4 make sure you put an & before the publiciable
	...
	function registerUriFunction($mFunction, $sRequestURI, $sRequestType=XAJAX_POST)
	{
		if (is_array($mFunction)) {
		    ...
		    {
			    $sRequestURI = 'http://'.$_SERVER['HTTP_HOST'].'/'.LOCALE.'/'.$params[2].'/xajaxcontext/'.$params[1];
			}
			else
			{
			    $sRequestURI = 'http://'.$_SERVER['HTTP_HOST'].'/'.LOCALE.'/'.$params[1].'/'.$params[2].'/xajaxcontext/'.$params[1];
			}
		    $this->aFunctions[$mFunction[0]] = 1;
			$this->aFunctionRequestTypes[$mFunction[0]] = $sRequestType;
			...
			if ( $_SERVER['HTTP_HOST'] != BASE_HTTP_HOST ) {
				 ...
				 //$sRequestURI = 'http://'.$_SERVER['HTTP_HOST'].'/'.$params[1].'/'.$params[2].'/xajaxcontext/';
				 $sRequestURI = 'http://'.$_SERVER['HTTP_HOST'].'/'.$params[1].'/'.$params[2].'/xajaxcontext/'.$params[1];
				 // changed because with "www" didn't work			
			    // $sRequestURI = 'http://'.$_SERVER['HTTP_HOST'].'/'.$params[2].'/xajaxcontext/'.$params[1];
			} else {
			    $sRequestURI = 'http://'.$_SERVER['HTTP_HOST'].'/'.$params[1].'/'.$params[2].'/xajaxcontext/'.$params[1];
			}
			$this->aFunctions[$mFunction] = 1;
	...
	/**
	 * Returns the current URL based upon the SERVER publics.
	 *
	 * @access private
	 * @return string
	 */
